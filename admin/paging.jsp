<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
String actionPath = request.getParameter("actionPath");//airplainlist.jsp가 넘어옴
...
//리스트타입 가져오기(airplain , package , hotel)
...
<form name ="frmPaging" method="get" action="<%=actionPath%>">
	...
	<%
	if(listType.equals("airplain")){
		%>
		<input type="hidden" name="showPage" value="admin_airplainList">
		<%
	}else if(listType.equals("package")){
		%>
		<input type="hidden" name="showPage" value="admin_packageList">
		<%
	}else if(listType.equals("hotel")){
		%>
		<input type="hidden" name="showPage" value="admin_hotelList">
		<%
	}else{
		%>
		<input type="hidden" name="showPage" value="admin_airplainList">
